...
- `*_*.qmd`：各章の本文（Rコードと解説）
- 章ファイル（.qmd）はルート直下に配置
- `_scripts/`：ビルド補助・正規化・保守用の補助スクリプト

## フォルダ整理の方針（今後）

- 章はルート直下に統一しました（過去に一時的に `chapters/` を使用していた名残がある場合があります）
- 図表や補助データは `data/` と将来的な `assets/` 配下に整理
- 日常的なローカル生成物（`.html`, `*_cache/`, `index.tex`, ログ類など）は `.quartoignore` で除外
- 一方で、再現可能な公開物として必要な `_freeze/` や章ごとの一部図版資産はリポジトリに保持
- `_book/` は最終出力ディレクトリで、通常は再レンダリングで更新
